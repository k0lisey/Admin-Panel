...
<div id="alert" class="animated bounceInDown"></div>
    ...
    <div class="content_add">
        <div class="left_panel">
            ...
            <div class="action_key">
                ...
                <a class="active" href="add_key.php">
        ...
        <div class="add_user_main_panel">
            ...
            <div class="add_users">
                <div class="user_id flex align-items">
                   <h1>Ключ</h1>
                    <input id="generation_key" type="text">
                </div>
                <button id="add_key" type="button" class="add_btn">Добавить</button>
                <button id="create_key" type="button" class="add_btn">Сгенерировать</button>
            </div>
        </div>
    </div>
  ...
  $('#add_key').click(function(){
    var key_value = $('#generation_key').val();

        $.ajax({
            url: 'add_key_data.php',
            method: 'POST',
            data: {
                key_value: key_value
            }
        ...
        .done(function(msg) {
            if (msg == true) {
                show_alert_suc('<div><i class="far fa-check-circle"></i></div><div><p>Ключ успешно добавлен</p></div>');
...
$('#create_key').click(function(){
    ...
    $('#generation_key').val(key_generation(20));

    $('#generation_key').keyup(function(){
        ...
        else
        {
            ...
            $('#add_key').attr('disabled', true);      
        }
    })

    if($('#generation_key').val().length == 20){
        $('#add_key').css({
            'background': '#28a745',
            'cursor': 'pointer'
        });
...
function key_generation(key_int) {
    chrs = 'abdehkmnpswxzABDEFGHKMNPQRSTWXZ123456789';
    var str = '';

    for (var i = 0; i < key_int; i++) {
